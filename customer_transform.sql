-- Stored Procedure for cleanup and transformation and called from task automatically, it is not pulling data from tables, it is pulling data from streams which is getting incremental data

--check if new data insert, exisitng update merge
--when matched update, when not matched insert

CREATE OR REPLACE PROCEDURE transform_customer()
RETURNS STRING
LANGUAGE SQL
AS
$$
